...
const COLORS = ['#f59e0b', '#6366f1', '#6366f1', '#f59e0b', '#6366f1', '#6366f1', '#f59e0b', '#6366f1', '#818cf8', '#818cf8', '#fbbf24', '#fbbf24'];
            ...
            <div className="bg-card border border-card-border rounded-2xl p-6 w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Echo, Pulse, Nyx" autoFocus className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none focus:border-amber-500/50 transition-all placeholder-dim-text" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select value={role} onChange={e => setRole(e.target.value as AgentRole)} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none appearance-none cursor-pointer focus:border-amber-500/50 transition-all">
                                {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="text-xs font-bold text-secondary-text uppercase tracking-widest block mb-2">Reports To</label>
                            <select value={parentId || ''} onChange={e => setParentId(e.target.value || null)} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none appearance-none cursor-pointer focus:border-amber-500/50 transition-all">
                    ...
                    <div>
                        ...
                        <select value={model} onChange={e => setModel(e.target.value)} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none appearance-none cursor-pointer focus:border-amber-500/50 transition-all">
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {COLORS.map(c => (
                                <button key={c} onClick={() => setColor(c)} className={`w-8 h-8 rounded-lg transition-all shadow-sm ${color === c ? 'ring-2 ring-amber-400 ring-offset-2 ring-offset-card scale-110' : 'hover:scale-105 hover:ring-2 hover:ring-white/20 hover:ring-offset-1 hover:ring-offset-card'}`} style={{ background: c }} />
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {ICONS.map(ic => (
                                <button key={ic.id} onClick={() => setIconId(ic.id)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-sm ${iconId === ic.id ? 'bg-black/10 ring-2 ring-amber-400' : 'bg-black/5 hover:bg-black/10 border border-divider hover:border-amber-500/30'}`}>
                ...
                <div className="flex gap-4 mt-6">
                    ...
                    <button onClick={handleCreate} disabled={!name.trim()} className="flex-1 bg-amber-500 text-black py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-amber-400 transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(245,158,11,0.3)] disabled:shadow-none">Create Agent</button>
                </div>
            </div>
        </div>
    );
