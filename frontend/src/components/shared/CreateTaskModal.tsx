            ...
            <div className="bg-card border border-card-border rounded-2xl p-6 w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Research competitor pricing pages" autoFocus className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none focus:border-emerald-500/50 transition-all placeholder-dim-text" />
                    </div>

                    {/* Description */}
                    <div>
                        <label className="text-xs font-bold text-secondary-text uppercase tracking-widest block mb-2">Description</label>
                        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Details, context, expected output..." rows={3} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none focus:border-emerald-500/50 transition-all resize-none placeholder-dim-text" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select value={assigneeId} onChange={e => setAssigneeId(e.target.value)} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none appearance-none cursor-pointer focus:border-emerald-500/50 transition-all">
                                {AGENTS.map(a => (
                            ...
                            <div className="flex gap-1">
                                {(['low', 'medium', 'high', 'critical'] as TaskPriority[]).map(p => (
                                    <button key={p} onClick={() => setPriority(p)} className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all ${priority === p
                                        ? p === 'critical' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30 shadow-sm'
                                            : p === 'high' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30 shadow-sm'
                                                : p === 'medium' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30 shadow-sm'
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select value={projectId} onChange={e => { setProjectId(e.target.value); setMilestoneId(''); }} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none appearance-none cursor-pointer focus:border-emerald-500/50 transition-all">
                        ...
                        <div>
                            ...
                            <select value={milestoneId} onChange={e => setMilestoneId(e.target.value)} className="w-full bg-black/5 border border-divider rounded-xl p-3 text-sm text-main-text outline-none appearance-none cursor-pointer focus:border-emerald-500/50 transition-all">
                ...
                <div className="flex gap-4 mt-8">
                    ...
                    <button onClick={handleCreate} disabled={!title.trim()} className="flex-1 bg-emerald-500 text-black py-3 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-emerald-400 transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(16,185,129,0.3)] disabled:shadow-none">Create Task</button>
                </div>
            </div>
        </div>
    );
